...
import { 
  Recycle, Plus, Trash2, ArrowUpRight, ClipboardList, 
  AlertTriangle, FileText, UploadCloud, History, ShieldCheck, Sparkles 
} from 'lucide-react';
...
export default function Waste() {
  ...
  const [wasteType, setWasteType] = useState('NON_HAZARDOUS');
  ...
  const wasteTypesList = ['Plastic', 'Paper', 'Metal', 'Glass', 'Organic', 'Chemical', 'Electronic', 'E-Waste', 'Construction', 'Other'];
  const treatments = ['Recycling', 'Reuse', 'Recovery', 'Composting', 'Incineration', 'Landfill', 'Other'];
  ...
  const fetchRecords = async () => {
    ...
    try {
      const url = statusFilter === 'ALL' 
        ...
        : `/api/environment/waste?status=${statusFilter}`;
  ...
  const handleValidateOnly = async () => {
    try {
      ...
      if (data.isValid) {
        setActionMessage({ type: 'success', text: 'Validation Successful: Waste data complies with circularity standards.' });
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setActionMessage({ type: 'success', text: 'Waste record draft saved successfully to database.' });
        resetForm();
        fetchRecords();
      } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const json = await res.json();
      if (res.ok) {
        setActionMessage({ type: 'success', text: 'Waste record submitted successfully for verification!' });
        resetForm();
        fetchRecords();
  ...
  const handleEditRecord = (record) => {
    ...
    if (record.wasteType) setWasteType(record.wasteType);
    setQuantity(record.quantity || record.amount || '');
    if (record.unit) setUnit(record.unit);
    if (record.disposalMethod) setDisposalMethod(record.disposalMethod);
    if (record.vendor) setVendor(record.vendor);
  ...
  const handleResubmitRecord = async (record) => {
    ...
    try {
      ...
      const json = await res.json();
      if (res.ok) {
        setActionMessage({ type: 'success', text: 'Waste record resubmitted successfully for verification.' });
        fetchRecords();
      } else {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`/api/environment/waste/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const totalWasteKg = records.reduce((acc, r) => {
    const q = r.quantity || r.amount || 0;
    return acc + (r.unit === 'Tonnes' ? q * 1000 : q);
  }, 0);

  const divertedKg = records
    .filter(r => ['Recycling', 'Reuse', 'Recovery', 'Composting'].includes(r.disposalMethod))
    .reduce((acc, r) => {
      const q = r.quantity || r.amount || 0;
      return acc + (r.unit === 'Tonnes' ? q * 1000 : q);
    }, 0);

  const diversionRate = totalWasteKg > 0 ? (divertedKg / totalWasteKg) * 100 : 0;

  return (
    <div className="pl-64 pr-8 py-8">
      <Navbar title="Waste Diversion & Hazardous Materials" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 mt-6">
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex items-center justify-between">
          <div>
            <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Total Waste Generated</p>
            <h3 className="text-2xl font-bold text-slate-800 mt-1">{(totalWasteKg / 1000).toFixed(2)} Tonnes</h3>
          </div>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex items-center justify-between">
          <div>
            <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Diverted from Landfill</p>
            <h3 className="text-2xl font-bold text-slate-800 mt-1">{(divertedKg / 1000).toFixed(2)} Tonnes</h3>
          </div>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex items-center justify-between">
          <div>
            <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Diversion Rate</p>
            <h3 className="text-2xl font-bold text-slate-800 mt-1">{diversionRate.toFixed(1)}%</h3>
          </div>
          <span className="text-xs font-semibold px-2.5 py-1 bg-blue-50 text-blue-700 rounded-lg">Target: 70%+</span>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        
        {/* Table List of Logs & Verification Queue */}
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            ...
            <div className="px-6 py-5 border-b border-slate-100 bg-slate-50 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              <div>
                <h2 className="font-bold text-slate-800">Waste Manifests & Disposal Queue</h2>
            ...
            {loading ? (
              <div className="p-8 text-center text-slate-500">Loading waste records...</div>
            ) : records.length === 0 ? (
              <div className="p-8 text-center text-slate-400">
                No waste logs found for {statusFilter === 'ALL' ? 'this scope' : `filter: ${statusFilter}`}.
              </div>
            ) : (
                    ...
                    {records.map((r) => (
                      <React.Fragment key={r._id}>
                        <tr className="hover:bg-slate-50 transition-colors">
                          ...
                          <td className="py-4 px-6">
                            <span className="font-bold text-slate-800 block">{r.category}</span>
                            <span className={`inline-block px-1.5 py-0.5 rounded text-[10px] font-semibold tracking-wider uppercase mt-0.5 ${
                              r.wasteType === 'HAZARDOUS' ? 'bg-red-50 text-red-700' : 'bg-green-50 text-green-700'
                            }`}>
                              {r.wasteType}
                            </span>
                          </td>
                          <td className="py-4 px-6 font-semibold text-slate-700">
                            <span>{r.disposalMethod}</span>
                            {r.vendor && (
                              <span className="block text-[10px] text-slate-400">Vendor: {r.vendor}</span>
                            )}
                          </td>
                          <td className="py-4 px-6 font-bold text-slate-900 text-sm">
                            {(r.quantity || r.amount || 0).toLocaleString()} {r.unit || 'kg'}
                          </td>
                          <td className="py-4 px-6">
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 sticky top-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-bold text-slate-900 flex items-center space-x-2">
                ...
                <span>{editingDraftId ? 'Edit Waste Manifest' : 'Log Waste Generation'}</span>
              </h2>
              {editingDraftId && (
                <button
                  type="button"
            ...
            {!canEdit ? (
              <div className="bg-yellow-50 text-yellow-800 p-4 rounded-xl text-sm border border-yellow-100">
                🔒 You do not have permissions to submit waste logs.
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Classification</label>
                    <select
                      value={wasteType}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Treatment Route</label>
                    <select
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Certified Vendor</label>
                    <input
                      type="text"
                      value={vendor}
                ...
                <div className="p-3.5 rounded-xl border border-slate-200 bg-slate-50">
                  <div className="flex items-center justify-between mb-1.5">
                    <label className="text-xs font-bold text-slate-700 uppercase tracking-wider">
                      Disposal Manifest <span className="text-amber-600 font-normal">(Mandatory)</span>
                    </label>
                    <button
                      type="button"
                    ...
                    >
                      {attachedEvidenceName ? 'Change' : 'Upload Manifest'}
                    </button>
                  </div>
                  {attachedEvidenceName ? (
                    ...
                    <p className="text-[11px] text-slate-500">
                      Attach hazardous waste transfer note, scale slip, or vendor receipt.
                    </p>
                  )}
                </div>
